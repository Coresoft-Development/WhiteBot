...
class DB {
  constructor(dbPath) {
    ...
    /* 2. BARU MIGRASI ⬇️ */
    const hasPhone    = this.db.prepare("SELECT 1 FROM pragma_table_info('users') WHERE name='phone'").get();
    const hasJoinDate = this.db.prepare("SELECT 1 FROM pragma_table_info('users') WHERE name='join_date'").get();
    if (!hasPhone)    this.db.exec(`ALTER TABLE users ADD COLUMN phone TEXT;`);
    if (!hasJoinDate) this.db.exec(`ALTER TABLE users ADD COLUMN join_date INTEGER DEFAULT (strftime('%s','now'));`);
  }
  ...
  getAllUsers() {
    return this.db.prepare('SELECT jid, name FROM users ORDER BY join_date DESC').all();
  }
  ...
  countMessages(jid) {
    return this.db.prepare('SELECT COUNT(*) as total FROM messages WHERE jid = ?').get(jid).total;
  }
